...
function renderMarkdown(content: string): string {
    return content
        ...
        .replace(/`([^`]+)`/g, '<code class="bg-gray-100 text-light-teal px-1 py-0.5 rounded text-xs font-mono">$1</code>')
        .replace(/```[\w]*\n([\s\S]*?)```/g, '<pre class="bg-gray-900 text-gray-100 rounded-xl p-4 text-xs font-mono overflow-x-auto my-3"><code>$1</code></pre>')
        .replace(/^- (.+)$/gm, '<li class="ml-4 list-disc text-gray-700 text-sm">$1</li>')
        .replace(/^> (.+)$/gm, '<blockquote class="border-l-4 border-light-teal pl-3 text-gray-500 italic text-sm my-2">$1</blockquote>')
        .replace(/\n{2,}/g, '<br class="my-2" />');
}
...
function parseQuizMarkdown(markdown: string): QuizQuestion[] {
    ...
    for (const block of blocks) {
        ...
        const options = lines.slice(1).map((line) => {
            ...
            if (trimmed.startsWith("* ")) return { text: trimmed.slice(2), correct: true };
            if (trimmed.startsWith("- ")) return { text: trimmed.slice(2), correct: false };
            return null;
...
function QuizCard({ question, options }: QuizQuestion) {
    ...
    const getStyle = (index: number, correct: boolean) => {
        if (selected === null) return "border-gray-200 hover:border-dark-teal cursor-pointer";
        ...
        <div className="flex flex-col gap-3 p-4 rounded-xl border border-gray-200 bg-white">
            ...
            {selected !== null && (
                <p className={`text-xs font-medium ${options[selected].correct ? "text-light-teal" : "text-red-500"}`}>
                    {options[selected].correct ? "✓ Correct!" : `✗ Correct answer: ${options.find(o => o.correct)?.text}`}
                </p>
            )}
        </div>
    );
        ...
        >
            ...
            >
                <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                    <h2 className="text-sm font-semibold text-gray-800">Markdown syntax guide</h2>
                ...
                <div className="p-5 flex flex-col gap-2">
                    {MARKDOWN_HINTS.map((hint) => (
                        <div key={hint.syntax} className="flex items-start gap-3">
                            ...
                            <span className="text-xs text-gray-500 pt-1">{hint.description}</span>
                        </div>
                    ))}
                </div>
            </div>
...
// --- Preview modal ---
function PreviewModal({ lecture, content, onClose }: {
    lecture: Lecture;
    content: string;
    onClose: () => void;
}) {
            ...
            >
                ...
                <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded-full border ${TYPE_STYLES[lecture.type]}`}>
                            {lecture.type}
                        </span>
                    </div>
                    <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-lg leading-none">✕</button>
                ...
                <div className="overflow-y-auto px-6 py-5 flex flex-col gap-4">
                    ...
                    {isQuiz && quizQuestions.length > 0 && (
                        <div className="flex flex-col gap-3 mt-2">
                            <p className="text-xs font-medium text-gray-400 uppercase tracking-wider">Questions</p>
...
export default function Page({ params }: PageProps) {
    ...
    const selectedLecture = MOCK_LECTURES.find((l) => l.id === selectedId) ?? null;
    ...
    function handleSubmit() {
        ...
        router.push(`/admin/create/${slug}/summary`);
    }
            ...
            <div className="flex justify-center items-start p-6 w-full min-h-screen bg-gray-50">
                <div className="max-w-[960px] min-w-[350px] w-full rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                    ...
                    <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                        <div>
                            <p className="text-xs text-gray-400 uppercase tracking-widest">Training</p>
                            <h1 className="text-base font-semibold text-gray-800 capitalize">{slug.replace(/-/g, " ")}</h1>
                        </div>
                    ...
                    <div className="flex gap-0 divide-x divide-gray-100">
                        ...
                        <div className="w-1/2 flex flex-col p-5 gap-4">
                            ...
                            <div className="rounded-xl overflow-hidden border border-gray-200 h-[180px] bg-gray-100 flex items-center justify-center">
                                ...
                                />
                                <span className="text-gray-400 text-sm absolute">No preview available</span>
                            </div>
                            ...
                            <div className="flex flex-col gap-2 flex-1">
                                    ...
                                    <div className="flex items-center gap-2">
                                        {selectedLecture && (
                                            <>
                                                ...
                                                <span className={`text-xs px-2 py-0.5 rounded-full border ${TYPE_STYLES[selectedLecture.type]}`}>
                                ...
                                <textarea
                                    ...
                                    placeholder={selectedLecture ? "Edit lecture markdown here..." : "← Select a lecture to start editing"}
                                        ...
                                        >
                                            <div className="flex items-center justify-between gap-2">
                                                <div className="flex items-center gap-2 min-w-0">
                                                    ...
                                                    <span className="text-sm font-medium text-gray-800 truncate">{lecture.title}</span>
                                                </div>
                                                <div className="flex items-center gap-1.5 shrink-0">
                                                    {isDirty && (
                                                        <span className="w-1.5 h-1.5 rounded-full bg-yellow" title="Unsaved changes" />
                                                    )}
                                                    <span className={`text-xs px-2 py-0.5 rounded-full border ${TYPE_STYLES[lecture.type]}`}>
                                                        {lecture.type}
                                                    </span>
